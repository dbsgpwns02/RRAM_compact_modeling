...
class NARX_Paper(nn.Module):
    def __init__(self, in_dim, z_dim=64): # z_dim=64 (기존)
        super().__init__()
        ...
        # [!] hr_lr_in_dim이 z_dim + 1 (65)로 복구됨
    ...
    def forward(self, x):
        # x = [v_scaled, dir, rate, W_hist...]
        
        # --- 1. 상태(alpha) 계산 ---
        z = self.z_backbone(x)
        r_ref_logit = self.head_ref(z)
        
        # --- 2. 저항(R_hr, R_lr) 계산 (원래대로 복구) ---
        v_scaled = x[:, 0:1]
        
        # [!] z와 v_scaled를 합쳐서 입력으로 사용
