...
# [필수] 1. SPICE PWL 생성 함수 import
try:
    from generate_pwl import generate_v_data_from_spice_pwl
    from generate_pwl import generate_sweep_v_data
except ImportError:
    print("ERROR: generate_pwl.py 파일에 함수가 없습니다.")
    sys.exit(1)

# [필수] 2. Verilog-A 익스포트 함수 import
try:
    from export_narx_to_veriloga_paper import export_veriloga_hspice_split
except ImportError:
    print("ERROR: export_narx_to_veriloga_paper.py 파일을 찾을 수 없습니다.")
    sys.exit(1)
...
# ------------------------------------------------------------
# [훈련에 필요한 함수들]
# (load_iv_csv, build_features_paper, NARX_Paper, predict_with_internals)
# ------------------------------------------------------------
def set_seed(seed):
    """모든 무작위성을 통제하는 함수"""
...
def main():
    ...
    # =========================================================
    # 1. 훈련 (TRI_CSV 사용, z-backbone 동결 적용)
    # =========================================================
    print(f"\n[Training] Loading {TRI_CSV.name} for training...")
    State_tri, V_tri, I_tri = load_iv_csv(TRI_CSV)
    
    # [수정] 데이터 증강(Augmentation)이 적용된 build_features_paper 사용
    ...
    # --- Step 2: R Modules (z_backbone 동결 적용) ---
    print(f"\n[Training Step 2] Training R Modules... ({EPOCHS_STEP2} epochs)")
    model.head_ref.requires_grad_(False)
    model.head_hr.requires_grad_(True)
    model.head_lr.requires_grad_(True)
    model.z_backbone.requires_grad_(False) # <<< Z-BACKBONE 동결

    # [수정] 데이터 증강이 적용된 X_tri_scaled 사용
    ...
    opt2 = optim.Adam(
        # list(model.z_backbone.parameters()) + # <<< Z-BACKBONE 동결
    ...
    torch.save({
        ...
        'ny': ny,
        'in_dim': X_tri.shape[1] # 모델 구조 복원에 필요
    }, CHECKPOINT_PATH)
